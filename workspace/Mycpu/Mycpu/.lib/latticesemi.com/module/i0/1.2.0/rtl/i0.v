...
module i0 (apb_pclk_i, 
    ...
    input apb_pclk_i ; 
    input apb_presetn_i ; 
    input [0:0] apb_s01_psel_slv_i ; 
    input [31:0] apb_s01_paddr_slv_i ; 
    input [0:0] apb_s01_pwrite_slv_i ; 
    input [31:0] apb_s01_pwdata_slv_i ; 
    input [0:0] apb_s01_penable_slv_i ; 
    output [0:0] apb_s01_pready_slv_o ; 
    output [0:0] apb_s01_pslverr_slv_o ; 
    output [31:0] apb_s01_prdata_slv_o ; 
    ...
    i0_ipgen_lscc_apb_interconnect #(.TOTAL_MASTER_CNT(2),
            .TOTAL_SLAVE_CNT(2),
            ...
            .S_M_PRIO_IDX({2'd2,
                2'd1})) lscc_apb_interconnect_inst (.apb_pclk_i(apb_pclk_i), 
                .apb_presetn_i(apb_presetn_i), 
                .apb_psel_slv_i({apb_s01_psel_slv_i[0],
                    apb_s00_psel_slv_i[0]}), 
                .apb_paddr_slv_i({apb_s01_paddr_slv_i[31:0],
                    apb_s00_paddr_slv_i[31:0]}), 
                .apb_pwrite_slv_i({apb_s01_pwrite_slv_i[0],
                    apb_s00_pwrite_slv_i[0]}), 
                .apb_pwdata_slv_i({apb_s01_pwdata_slv_i[31:0],
                    apb_s00_pwdata_slv_i[31:0]}), 
                .apb_penable_slv_i({apb_s01_penable_slv_i[0],
                    apb_s00_penable_slv_i[0]}), 
                .apb_pready_slv_o({apb_s01_pready_slv_o[0],
                    apb_s00_pready_slv_o[0]}), 
                .apb_pslverr_slv_o({apb_s01_pslverr_slv_o[0],
                    apb_s00_pslverr_slv_o[0]}), 
                .apb_prdata_slv_o({apb_s01_prdata_slv_o[31:0],
                    apb_s00_prdata_slv_o[31:0]}), 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_arbiter #(parameter K = 32, 
    ...
    generate
        if ((ARBITER_SCHEME == 3'b0)) 
            begin : genblk2
                ...
                // Unmasked Grant
                // -----------------
                i0_ipgen_lscc_apb_prio_arb #(.M(M)) u_lscc_apb_rr_gnt_unmasked (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                ...
                // Masked Grant
                // -----------------
                i0_ipgen_lscc_apb_prio_arb #(.M(M)) u_lscc_apb_rr_gnt_masked (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                ...
                // Mask Generation
                // -----------------
                i0_ipgen_lscc_apb_thermo_mask #(.M(M)) u_lscc_apb_rr_mask (//AUTOINSTPARAM*/c
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                        .mask_o(mask[(M - 1):0]), 
    ...
    generate
        if ((ARBITER_SCHEME == 3'b1)) 
            begin : genblk3
                ...
                // Priority Arbiter
                // -----------------
                i0_ipgen_lscc_apb_prio_arb #(.M(M)) u_lscc_apb_fp_gnt (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                        .gnt_o(gnt[(M - 1):0]), 
...
// ==============================================================================
module i0_ipgen_lscc_apb_prio_arb #(parameter M = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
// ==============================================================================
module i0_ipgen_lscc_apb_thermo_mask #(parameter M = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_arbmux #(parameter K = 32, 
        parameter TOTAL_MASTER_CNT = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter M_PRIO_WIDTH = clog2(TOTAL_MASTER_CNT), 
        parameter DATA_WIDTH = 32, 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_bus #(parameter K = 32, 
        parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
        parameter DATA_WIDTH = 32, 
    ...
    // APB Multiplexor
    // ---------------------
    i0_ipgen_lscc_apb_multiplexor #(.K(K),
            .M_ADDR_WIDTH(M_ADDR_WIDTH),
            .DATA_WIDTH(DATA_WIDTH),
            .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_crossbar #(parameter K = 32, 
        parameter F = 1, 
        parameter TOTAL_MASTER_CNT = 8, 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_decoder #(parameter K = 32, 
        parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
        parameter TOTAL_SLAVE_CNT = 32, 
    ...
    generate
        ...
        begin : genblk1
            i0_ipgen_lscc_apb_decoder_prim #(.K(K),
                    .F(F),
                    .M_ADDR_WIDTH(M_ADDR_WIDTH),
                    .FULL_DECODE_EN(FULL_DECODE_EN),
...
// ==============================================================================
module i0_ipgen_lscc_apb_decoder_prim #(parameter K = 32, 
        parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
    ...
    generate
        ...
        begin : genblk1
            i0_ipgen_lscc_apb_decoder_comp #(.K(K),
...
// ==============================================================================
module i0_ipgen_lscc_apb_decoder_comp #(parameter K = 32, 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_multiplexor #(parameter K = 32, 
        parameter M_ADDR_WIDTH = 32, 
        parameter DATA_WIDTH = 32, 
        parameter TOTAL_SLAVE_CNT = 32, 
        parameter OUTREG_EN = 0) (
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module i0_ipgen_lscc_apb_interconnect #(parameter K = 32, 
        parameter F = 1, 
        parameter TOTAL_MASTER_CNT = 8, 
        parameter TOTAL_SLAVE_CNT = 8, 
        parameter M_ADDR_WIDTH = 32, 
    ...
    generate
        ...
        if (((TOTAL_MASTER_CNT > 1) && (TOTAL_SLAVE_CNT > 1))) 
            begin : apb_crossbar
                ...
                // AHB-Lite Crossbar
                // ------------------
                i0_ipgen_lscc_apb_crossbar #(.K(K),
                        .F(F),
                        .TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                        .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
                        .M_ADDR_WIDTH(M_ADDR_WIDTH),
