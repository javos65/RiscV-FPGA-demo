...
module apb0_0 (apb_pclk_i, 
    ...
    apb0_0_ipgen_lscc_apb_interconnect #(.TOTAL_MASTER_CNT(1),
            .TOTAL_SLAVE_CNT(2),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_arbiter #(parameter K = 32, 
    ...
    generate
        if ((ARBITER_SCHEME == 3'b0)) 
            begin : genblk2
                ...
                // Unmasked Grant
                // -----------------
                apb0_0_ipgen_lscc_apb_prio_arb #(.M(M)) u_lscc_apb_rr_gnt_unmasked (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                ...
                // Masked Grant
                // -----------------
                apb0_0_ipgen_lscc_apb_prio_arb #(.M(M)) u_lscc_apb_rr_gnt_masked (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                ...
                // Mask Generation
                // -----------------
                apb0_0_ipgen_lscc_apb_thermo_mask #(.M(M)) u_lscc_apb_rr_mask (//AUTOINSTPARAM*/c
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                        .mask_o(mask[(M - 1):0]), 
    ...
    generate
        if ((ARBITER_SCHEME == 3'b1)) 
            begin : genblk3
                ...
                // Priority Arbiter
                // -----------------
                apb0_0_ipgen_lscc_apb_prio_arb #(.M(M)) u_lscc_apb_fp_gnt (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                        .gnt_o(gnt[(M - 1):0]), 
...
// ==============================================================================
module apb0_0_ipgen_lscc_apb_prio_arb #(parameter M = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
// ==============================================================================
module apb0_0_ipgen_lscc_apb_thermo_mask #(parameter M = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_arbmux #(parameter K = 32, 
        parameter TOTAL_MASTER_CNT = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter M_PRIO_WIDTH = clog2(TOTAL_MASTER_CNT), 
        parameter DATA_WIDTH = 32, 
...
always @* begin
 ...
 -----/\----- EXCLUDED -----/\----- */
    ...
    // -----------------------------
    apb0_0_ipgen_lscc_apb_arbiter #(.K(K),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_bus #(parameter K = 32, 
        parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
        parameter DATA_WIDTH = 32, 
    ...
    // APB Decoder
    // -----------------
    apb0_0_ipgen_lscc_apb_decoder #(.K(K),
            .F(F),
            .M_ADDR_WIDTH(M_ADDR_WIDTH),
            .FULL_DECODE_EN(FULL_DECODE_EN),
    ...
    // APB Multiplexor
    // ---------------------
    apb0_0_ipgen_lscc_apb_multiplexor #(.K(K),
            .M_ADDR_WIDTH(M_ADDR_WIDTH),
            .DATA_WIDTH(DATA_WIDTH),
            .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_crossbar #(parameter K = 32, 
        parameter F = 1, 
        parameter TOTAL_MASTER_CNT = 8, 
    ...
    // APB Arbiter-Multiplexor
    // -----------------------------
    apb0_0_ipgen_lscc_apb_arbmux #(.K(K),
            .TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
            .M_ADDR_WIDTH(M_ADDR_WIDTH),
            .DATA_WIDTH(DATA_WIDTH),
            .OUTREG_EN(OUTREG_EN),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_decoder #(parameter K = 32, 
        parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
        parameter TOTAL_SLAVE_CNT = 32, 
    ...
    generate
        ...
        begin : genblk1
            apb0_0_ipgen_lscc_apb_decoder_prim #(.K(K),
                    .F(F),
                    .M_ADDR_WIDTH(M_ADDR_WIDTH),
                    .FULL_DECODE_EN(FULL_DECODE_EN),
...
// ==============================================================================
module apb0_0_ipgen_lscc_apb_decoder_prim #(parameter K = 32, 
        parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
    ...
    generate
        ...
        begin : genblk1
            apb0_0_ipgen_lscc_apb_decoder_comp #(.K(K),
...
// ==============================================================================
module apb0_0_ipgen_lscc_apb_decoder_comp #(parameter K = 32, 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_multiplexor #(parameter K = 32, 
        parameter M_ADDR_WIDTH = 32, 
        parameter DATA_WIDTH = 32, 
        parameter TOTAL_SLAVE_CNT = 32, 
        parameter OUTREG_EN = 0) (
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module apb0_0_ipgen_lscc_apb_interconnect #(parameter K = 32, 
        parameter F = 1, 
        parameter TOTAL_MASTER_CNT = 8, 
        parameter TOTAL_SLAVE_CNT = 8, 
        parameter M_ADDR_WIDTH = 32, 
    ...
    generate
        ...
        if (((TOTAL_MASTER_CNT > 1) && (TOTAL_SLAVE_CNT > 1))) 
            begin : apb_crossbar
                ...
                // AHB-Lite Crossbar
                // ------------------
                apb0_0_ipgen_lscc_apb_crossbar #(.K(K),
                        .F(F),
                        .TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                        .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
                        .M_ADDR_WIDTH(M_ADDR_WIDTH),
